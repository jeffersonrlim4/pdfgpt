...
from utils import PASTA_ARQUIVOS, criar_chain_conversa

def config_page():
    st.header('Configuração', divider=True)
    
    model_name = st.text_input('Modifique o modelo OpenAI', value=get_config('model_name'))
    retrieval_search_type = st.text_input('Tipo de retrieval', value=get_config('retrieval_search_type'))
    retrieval_kwargs = st.text_input('Tipo de parâmetros de retrieval', value=json.dumps(get_config('retrieval_kwargs')))
